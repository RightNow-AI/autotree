...
\textbf{No end-to-end model-performance result is reported in this draft.}
The bundled ThoughtBench tasks and every plotted value are synthetic contract
fixtures.  They validate the end-to-end reporting path, but they cannot
support a claim about accuracy, cost, latency, KV reuse, or throughput.
GPU-scale end-to-end measurements and real benchmark traces are pending.

\subsection{Measured hardware validation}

One class of real measurement does exist: a kernel-level validation pass on a
single NVIDIA A100 SXM4 40GB (torch 2.11 cu128, Triton 3.6).  The fused
Triton tree-attention decode kernel passes 106 parity tests against the CPU
reference implementation, and token-level greedy parity against stock
HuggingFace generation holds for Qwen3-8B in bfloat16 on CUDA.  A synthetic
decode microbenchmark of the kernel alone (\texttt{bench\_decode}, 200
iterations, bfloat16) scales near-linearly with branch count: at context
length 128 it sustains 17.0k tokens/s with one branch and 539.3k tokens/s
with 32 branches ($31.6\times$), and at context length 2048 it reaches
$16.4\times$ at 32 branches.  This measurement supports the mechanism claim
--- that tree-structured attention makes additional reasoning branches nearly
free at the kernel level --- but it is not an end-to-end serving result and
does not by itself establish the cost-per-correct-answer targets below.

\subsection{First end-to-end measurement}

A first full-protocol ThoughtBench run exists (single H100 PCIe 80GB,
Qwen3-8B bfloat16 served through the reference engine; 25 numeric MATH-500
tasks at difficulty levels 1--3; three seeds; temperature 0.7).  Sequential
best-of-4 at 640 completion tokens per sample reached accuracy@1 of 30.7\%
and accuracy@4 of 56.0\% at 3{,}159 output tokens per correct answer.  A
beam-8 tree capped at 1{,}920 total budget tokens reached accuracy@1 of
21.3\% at 5{,}011 tokens per correct answer, while measuring a
\textbf{KV-reuse ratio of 8.79$\times$} --- direct end-to-end confirmation
that tree execution shares prefix KV across branches at scale.

The quality result is deliberately reported as a negative: with the default
log-probability winner selection and thin per-branch budgets
($\approx$240 tokens per branch), tree search \emph{loses} to independent
sampling on tasks this model can already solve.  The architecture's
value-scorer interface exists precisely to replace that selection rule, and
this measurement establishes the baseline any value-guided configuration
must beat.  AIME-difficulty tasks were attempted first and were infeasible
at reference-engine decode speeds (every sample truncated mid-reasoning at
1{,}024 tokens); production-rate serving is a prerequisite for
frontier-difficulty rows.
